...
export default function MessageList({ messages, currentUserId, isLoading, messagesEndRef, onReaction, onReply }: MessageListProps) {
  ...
  const getReplyMessage = (replyToId: string) => {
    const replyMsg = messages.find(msg => msg.id === replyToId)
    console.log('🔍 Finding reply message:', { replyToId, found: !!replyMsg, totalMessages: messages.length })
    return replyMsg
  }
    ...
    return (
      <div className="flex items-center justify-center h-full bg-gradient-to-br from-rose-50 via-pink-50 to-purple-50">
        <div className="text-center p-4 sm:p-8">
          <div className="w-16 h-16 sm:w-20 sm:h-20 bg-pink-100 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
            <svg className="w-8 h-8 sm:w-10 sm:h-10 text-pink-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
  ...
  return (
    ...
    >
      {messages.map((message, index) => {
        ...
        if (message.replyTo) {
          console.log('💬 Message has reply:', { messageId: message.id, replyTo: message.replyTo, replyMessage: replyMessage?.content })
        }
        ...
        return (
          ...
          >
            ...
            {!isOwn && (
              <div className={`w-8 h-8 sm:w-9 sm:h-9 flex-shrink-0 ${showAvatar ? 'opacity-100' : 'opacity-0'}`}>
                {showAvatar && (
                  <UserAvatar
            ...
            <div className={`max-w-[80%] sm:max-w-[70%] md:max-w-md lg:max-w-lg relative group`}>
              ...
              <div className="absolute -top-10 right-0 opacity-0 group-hover:opacity-100 transition-all duration-200 flex gap-1 bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl p-1.5 z-10 border border-rose-100">
                ...
                <motion.button
                  ...
                  title="React"
                >
                  <svg className="w-4 h-4 text-rose-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
              ...
              {/* Reply preview - INSIDE message bubble container */}
                ...
                {replyMessage && (
                  <motion.div 
                    ...
                    className={`mb-3 p-2 rounded-lg border-l-4 ${
                      isOwn 
                        ? 'bg-white/10 border-white/60 backdrop-blur-sm' 
                  ...
                  >
                    <div className={`flex items-center gap-1.5 text-xs font-semibold mb-1 ${
                      ...
                      <span>{replyMessage.senderId === currentUserId ? 'You' : replyMessage.sender.name}</span>
                    </div>
                    <div className={`text-xs line-clamp-2 pl-5 ${
